...
function pokeModal(pokemons) {
    ...
    pokemonCards.forEach(card => {
        card.addEventListener('click', (e) => {
            pokemonClicked = e.currentTarget.getElementsByClassName('name')[0].innerText.toLowerCase();
            let pokemonObject = pokemons.filter((pokemon) => pokemon.name === pokemonClicked);

            if(pokemonClicked === pokemonObject[0].name) {
                let pokeObj = pokemonObject[0];
                console.log(pokeObj);
                let html = '';

                html += '<div class="modal fade" id="'+ pokemonClicked +'Modal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">'
                html += '  <div class="modal-dialog modal-dialog-centered">'
                html += `  <div class="modal-content ${pokeObj.type}">`
                html += '  <div class="modal-header">'
                html += '  <h1 class="modal-title" id="staticBackdropLabel">'+ pokemonClicked +'</h1>'
                ...
                html += '                            </div>'
                html += `                   <img src="${pokeObj.photo}" alt="${pokemonClicked}" class="img-fluid">`
                ...
                let modalElement = document.getElementById(`${pokemonClicked}Modal`);
                if(modalElement) {
                    const modal = new bootstrap.Modal(modalElement);
                    modal.show();
                } else {
                    console.error(`Modal para ${pokemonClicked} não encontrado!`);
                }
            }
        })
    });
